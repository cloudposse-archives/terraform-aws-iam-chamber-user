...
Terraform Module to provision a basic IAM system user with access to SSM+KMS secrets, suitable for CI/CD Systems
...
```hcl
data "aws_iam_policy_document" "chamber_user_policy" {
  statement {
    actions = [
...
module "chamber_user" {
  ...
  stage     = "dev"
  name      = "chamber"
  policy    = "${data.aws_iam_policy_document.chamber_user_policy.json}"
}
```
