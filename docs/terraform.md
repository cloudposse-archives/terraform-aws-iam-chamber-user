...
| kms_key_arn | ARN of the KMS key which will decrypt secret strings | string | - | yes |
| name | Application or solution name (e.g. `app`) | string | - | yes |
| namespace | Namespace (e.g. `cp` or `cloudposse`) | string | - | yes |
| path | Path in which to create the user | string | `/` | no |
| ssm_actions | Actions to allow in the policy | list | `<list>` | no |
| ssm_resources | Resources to apply the actions specified in the policy | list | `<list>` | no |
